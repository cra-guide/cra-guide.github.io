...
in packages on npm and other registries, it is time we start treating
dependencies as potentially dangerous.
...
npm based malware infections.

Another precaution to limit the impact of such attacks is to use sandboxed
development environments.
One solution is to use a cloud development environment (CDE), such as: [GitHub
...
In addition, tokens can be created with a short expiration, although can create
some inconvenience.
...
might be a good idea to have a security conscious person review the CI/CD
pipeline.
Avoid running untrusted code as part of the pipeline.
